    ...
    for room in rooms:
        ...
        while clock < 1440:
            ...
            if clock + dur_hours * 60 + dur_minutes < 1440:
                start = "'{}:{}'".format(clock // 60, clock % 60)
                data.append([room[1],
                             room[0],
                             film[0],
                             start,
                             "dbo.GetEndTime({}, {})".format(film[0], start)])

            clock += dur_hours * 60 + dur_minutes + 15
...
def ddl_session_writer(f, films, rooms):
    ...
    for line in data:
        f.write("INSERT INTO Session VALUES (")
        n = len(line)
        for i in range(n - 1):
            f.write(str(line[i]) + ', ')
        f.write(str(line[n - 1]) + ');\n')
...
def main():
    f = open('/home/aleksiks/prog/BD/ddls/Filling-ddl.sql', 'w')
    ddl_cinema_writer(f)
